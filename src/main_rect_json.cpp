...
int main(int argc, char *argv[]){
    ...
    if(argc == 2){
        std::cerr<<"Usage of : "<< argv[1] << " <json_file> as scne description"<<std::endl;
        ...
        float view_height = camera["height"].get<float>(); //used directely
        ...
        float focal = camera["focal"].get<float>(); //used directely
        //adding to scene
        ...
        std::ofstream myfile;
        char buffer[100];
        sprintf(buffer,"image_of_%s.ppm",argv[1]);
        myfile.open(buffer);
        myfile<<"P3\n"<<width<<" "<<height<<"\n255\n";
    ...
    Point the_corner_rect= Point(9.0,3.0,-10.0);
    vec plageinf = vec(9.0,3.0,-10.0);
    vec plagesup = vec(11.0,5.0,-12.0);
    color the_rect_color(0.0,0.0,2.0);
    rectangle the_rect(the_corner_rect, plageinf, plagesup, the_rect_color);
    ...
    Shapes.add(make_shared<plan> (the_plan));
    Shapes.add(make_shared<rectangle> (the_rect));
